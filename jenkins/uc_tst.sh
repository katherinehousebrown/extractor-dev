...
export IMG_NAME="ods/extractor:latestUcTst"
export MACH_NAME="extractor-uc-tst"
export ARCHIVE_NAME="extractor-UC-TST.tgz"
...
#docker exec extractor-uc-tst bash -c 'cd /extractor; nose2 > nose2 --plugin nose2.plugins.junitxml --junit-xml'
#docker exec extractor-uc-tst bash -c 'newman run -e /extractor/test/local.postman_environment.json /extractor/test/guid.postman_collection.json > /extractor/test/newman.out'
#Copy Reports to Jenkins Server
echo "Copying Junit Reports"
#docker cp extractor-uc-tst:/extractor/nose2-junit.xml nose2-junit.xml
echo ""
echo ""
echo ""
...
aws s3 cp cloudFormation/tst/east_uc/start.sh s3://${BCKT_EAST}/jenkins/artifacts/builds/deploy/ods-extractor/uc/start.sh
aws s3 cp cloudFormation/tst/east_uc/UC_EAST.yaml s3://${BCKT_EAST}/jenkins/artifacts/builds/deploy/ods-extractor/uc/UC_EAST.yaml

aws s3 cp ${ARCHIVE_NAME} s3://${BCKT_WEST}/jenkins/artifacts/builds/deploy/ods-extractor/uc/${ARCHIVE_NAME}
aws s3 cp cloudFormation/tst/west_uc/start.sh s3://${BCKT_WEST}/jenkins/artifacts/builds/deploy/ods-extractor/uc/start.sh
aws s3 cp cloudFormation/tst/west_uc/UC_WEST.yaml s3://${BCKT_WEST}/jenkins/artifacts/builds/deploy/ods-extractor/uc/UC_WEST.yaml
